	...
	const pairs = inputWords.reduce((acc, word, wordIndex) => {
		...
		wordArr.some((_, letterIndex) => {
			...
			if (isPalindrome(prefixToCheck)) {
				suffixToCheck in dictionary &&
					dictionary[suffixToCheck] !== wordIndex &&
					acc.push([ wordIndex, dictionary[suffixToCheck] ]);
				prefixToCheck in dictionary &&
					dictionary[prefixToCheck] !== wordIndex &&
...
const _ = require('underscore');
const problem3 = (input, k) =>
	_.range(k).reduce((acc, _v, offset) => {
		// Extract the pattern by shifting & running the modulo at specififed indexes
		const line = input
			.split('')
			.map(
				(val, index) =>
					(index - offset) % (2 * k - 2) === 0 || (index - (2 * k - 2 - offset)) % (2 * k - 2) === 0
						? val
						: ' '
			);
		line.push('\n');
